...
\hline
Kubernetes & 2018 -- Amazon Elastic Container Service for Kubernetes \cite{siteAmazonBlogEks} & 2018 -- Google Kubernetes Engine \cite{siteGoogleBlogKubernetes}  & 2017 -- Azure Container Service \cite{siteMicrosoftBlogAks}\\
...
\item W procesie wytwarzania oprogramowania gwarantuje użycie identycznych wersji zależności na środowisku deweloperskim, testowym oraz produkcyjnym.
...
\hline
minimalne wymagania -- 40 GB HDD, 8 GB RAM, 4 rdzenie CPU & 2 GB RAM, 2 rdzenie CPU. Działa na Raspberry PI \cite{siteKubernetesOnRaspberry} \\
\hline
...
% do tego momentu kontenery jawią się jako antidotum na przywiązanie do wybranego dostawcy usług chmurowych, jako te co dają wolność uzywania takich technologii jakich się chce a nie jakieś DynamoDb
% niestety jak chce się ich używać to trzeba miec jakąś wiedzę taką poza pisaniem sobie aplikacji w jakimś języku programowania, no bo trzeba gdzieś zainstalować tego kubernetes, jakoś go skonfigurować
% nawet cloud foundry trzeba najpierw zainstalować a przeciez miało byc tak fajnie że tylko piszę aplikację
% i tadaaaa chmury publiczne wypuszczają Container As A Service tak że można pisać w kontenerach i mieć resztę w d
