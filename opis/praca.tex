...
\addbibresource{bibliography/siteGartnerTrends}
...
\begin{itemize}
\item rozwój chmur publicznych (sekcja \ref{chmuryPubliczne}),
\item rozwój rozwiązań opartych o kontenery (sekcja \ref{konteneryzacja}),
\item rozwój technik programistycznych wyznaczających najlepsze praktyki tworzenia aplikacji dla chmury (sekcja \ref{mikroserwisy}),
\item rozwój technologii bazodanowych dla systemów rozproszonych oraz metod analizy danych (sekcja \ref{nosql}),
\item rozwój technologii przyśpieszających dostęp do usług chmurowych (sekcja \ref{fog}).
\end{itemize}

\subsection{Rozwój chmur publicznych} \label{chmuryPubliczne}
...
\subsection{Konteneryzacja i DevOps} \label{konteneryzacja}
...
\subsection{Mikrousługi i serverless} \label{mikroserwisy}
...
\subsection{Bazy NoSQL, NewSQL i rozwiązania dla Big Data} \label{nosql}
...
\subsection{CDN i Fog Computing} \label{fog}
...
Za sprawą rosnącego znaczenia \textbf{Internetu Rzeczy}, przed technologiami w modelu chmury obliczeniowej stoją kolejne wyzwania. Wprawdzie Internet Rzeczy znajduje użycie już teraz, czego dobrym przykładem są inteligentne zegarki. Wizja \textit{networked society} obejmuje jednak znacznie więcej, np. korzystanie z samoprowadzących się samochodów (rysunek \ref{fig:fog-samochod}) czy wspomaganie opieki zdrowotnej przez choćby automatyczne powiadomienie rodziny o zawale serca u pacjenta. Tego rodzaju usługi wymagają dodatkowej warstwy, która podobnie jak CDN znajdowałaby się bliżej użytkownika niż centrum danych, ale potrafiłaby także wstępnie przetworzyć dane czy też zwrócić wynik bez angażowania do tego chmury.
...
Powszechnie korzysta się z atrybutów określonych przez amerykański Narodowy Instytut Standaryzacji i Technologii w 2011 r., zwanych \textbf{modelem NIST}. Wyróżnia on cechy zasadnicze, modele dostarczania usług i formy wdrożenia. \cite{nistArticle, ccCambridge}
...
\end{itemize}
W literaturze i innych publikacjach często można spotkać się także z innymi podobnymi zwrotami w formie \textit{,,... as a Service''}, jednak zawsze jest to podkategoria dla tych powyższych. IaaS, PaaS i SaaS stanowią następujące po sobie warstwy.
...
\textbf{IaaS} to wirtualne zasoby udostępnione jako usługa. Ponieważ wirtualizacja zasobów stanowi podstawową technikę stosowaną w modelu chmury, IaaS występuje zawsze gdy są udostępniane np. serwery wirtualne, dyski w chmurze czy sieci wirtualne. Przykład podgrupy: NaaS (\textit{Network} -- sieć).
...
\item CaaS (\textit{Container} -- kontener) -- wdrażanie aplikacji opakowanych w kontener,
...
Warstwa \textbf{SaaS} obejmuje gotowe do użycia aplikacje. Na tej warstwie wszystko znajduje się pod kontrolą dostawcy usługi. Jest to najbardziej dojrzały model dostarczania usługi chmurowej, był stosowany już w 1999 r. przez Salesforce. Do rozwiązań SaaS należą m.in. popularne aplikacje dostępne przez przeglądarkę internetową: Gmail, Dropbox, Microsoft Office 365. Przykładowe podgrupy to:
\begin{itemize}
...
\item rozwój w kierunku IoT - edge i fog przy użyciu kontenerów
\item ja bym widziała jakąś bazę, która raz na zawsze rozwiąże problem ,,jaką bazę wybrać''. Był na to fajny rysunek w jednej książce
\item serverless -- bo to co jest to na pewno nie jest ostatnie słowo w tym temacie...
\end{itemize}
